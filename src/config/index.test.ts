...
const baseEnv = (overrides: Record<string, string | undefined> = {}): NodeJS.ProcessEnv => ({
  MCP_CLAUDE_HOUSEKEEPING_PATH: '/tmp/housekeeping',
  ...overrides
})

describe('MCP_CLAUDE_HOUSEKEEPING_PATH', () => {
  it('expands a leading ~/ to the user home directory', () => {
    const cfg = loadConfig(baseEnv({ MCP_CLAUDE_HOUSEKEEPING_PATH: '~/some-housekeeping' }))
    expect(cfg.housekeepingPath).toBe(path.resolve(path.join(os.homedir(), 'some-housekeeping')))
  })

  it('leaves an absolute path unchanged', () => {
    const cfg = loadConfig(baseEnv({ MCP_CLAUDE_HOUSEKEEPING_PATH: '/tmp/explicit-housekeeping' }))
...
describe('MCP_CLAUDE_HOUSEKEEPING_ACCESS_LEVEL', () => {
  ...
  it('defaults to read when empty/whitespace', () => {
    expect(loadConfig(baseEnv({ MCP_CLAUDE_HOUSEKEEPING_ACCESS_LEVEL: '   ' })).accessLevel).toBe('read')
  })

  it('accepts write', () => {
    expect(loadConfig(baseEnv({ MCP_CLAUDE_HOUSEKEEPING_ACCESS_LEVEL: 'write' })).accessLevel).toBe('write')
  })

  it('accepts destructive', () => {
    expect(loadConfig(baseEnv({ MCP_CLAUDE_HOUSEKEEPING_ACCESS_LEVEL: 'destructive' })).accessLevel).toBe('destructive')
  })
...
describe('MCP_CLAUDE_HOUSEKEEPING_AUDIT_LOG', () => {
  it('defaults to writes when unset', () => {
    expect(loadConfig(baseEnv()).auditLogMode).toBe('writes')
  })

  it('accepts off / all', () => {
    expect(loadConfig(baseEnv({ MCP_CLAUDE_HOUSEKEEPING_AUDIT_LOG: 'off' })).auditLogMode).toBe('off')
    expect(loadConfig(baseEnv({ MCP_CLAUDE_HOUSEKEEPING_AUDIT_LOG: 'all' })).auditLogMode).toBe('all')
  })
...
describe('audit-log path + rotation knobs', () => {
  it('defaults the audit-log path under the housekeeping dir', () => {
    const cfg = loadConfig(baseEnv({ MCP_CLAUDE_HOUSEKEEPING_PATH: '/tmp/hk' }))
    expect(cfg.auditLogPath).toBe(path.resolve('/tmp/hk', 'audit', 'audit.jsonl'))
  })

  it('honours an explicit audit-log path (with ~ expansion)', () => {
    const cfg = loadConfig(baseEnv({ MCP_CLAUDE_HOUSEKEEPING_AUDIT_LOG_PATH: '~/logs/audit.jsonl' }))
  ...
  it('throws on a negative max bytes', () => {
    expect(() => loadConfig(baseEnv({ MCP_CLAUDE_HOUSEKEEPING_AUDIT_LOG_MAX_BYTES: '-5' }))).toThrow(/MCP_CLAUDE_HOUSEKEEPING_AUDIT_LOG_MAX_BYTES.*non-negative/)
  })
})
