...
import { daysAgo, duBytes, isNodeError, pathExists, readJsonIfExists } from './utils.js'

const DAY_MS = 1000 * 60 * 60 * 24

/* ==================== Check 1: storage summary ==================== */

export const storageSummary = async (args: { flag_size_gb: number; flag_session_count: number }) => {
  ...
  const sessionEntries = await Promise.all(sessionDirs.map(async (e) => ({ name: e.name, bytes: await duBytes(path.join(ROOT_PATH, e.name)) })))
...
export const listObsolete = async (args: { older_than_days: number; flag_count: number; flag_size_mb: number }) => {
  ...
  for (const name of sessionJsons) {
    ...
    if (stat.mtime.getTime() < cutoff) {
      ...
      const dirBytes = (await pathExists(path.join(ROOT_PATH, dirName))) ? await duBytes(path.join(ROOT_PATH, dirName)) : 0
      obsolete.push({
        name: dirName,
...
export const artifactHealth = async (args: { flag_versions: number; flag_stale_days: number; flag_unstarred_idle_days: number }) => {
  const artifactsPath = path.join(ROOT_PATH, 'artifacts.json')
...
export const artifactPrune = async (args: { keep: number; dry_run: boolean }) => {
  const artifactsPath = path.join(ROOT_PATH, 'artifacts.json')
  const artifacts = (await readJsonIfExists<ArtifactRecord[]>(artifactsPath)) ?? []
  ...
  for (const a of toDelete) {
    const cachePath = path.join(ROOT_PATH, 'artifacts', `cache_${a.id}.json`)
...
export const obsoleteOutputs = async (args: { older_than_days: number }) => {
  const cutoff = Date.now() - args.older_than_days * DAY_MS
  const entries = await fs.readdir(ROOT_PATH, { withFileTypes: true })
  ...
  for (const name of sessionDirs) {
    ...
    try {
      const stat = await fs.stat(path.join(ROOT_PATH, `${name}.json`))
    ...
    const outputs = await listFilesWithSize(path.join(ROOT_PATH, name, 'outputs'))
    const uploads = await listFilesWithSize(path.join(ROOT_PATH, name, 'uploads'))
...
export const backupSummary = async (args: { flag_count: number; flag_size_mb: number }) => {
  const candidates: string[] = []
  // Look in ROOT_PATH and ROOT_PATH/backups for `.claude.json.backup.*`
  for (const dir of [ROOT_PATH, path.join(ROOT_PATH, 'backups')]) {
    try {
  ...
  const items = await Promise.all(
    candidates.map(async (p) => {
      ...
      return {
        path: path.relative(ROOT_PATH, p),
        bytes: stat.size,
...
export const memorySpacesSummary = async (args: { flag_files: number }) => {
  const spacesDir = path.join(ROOT_PATH, 'spaces')
  let entries: Dirent[]
  try {
...
export const pluginsInventory = async () => {
  const installedPath = path.join(ROOT_PATH, 'cowork_plugins', 'installed_plugins.json')
  const rpmManifestPath = path.join(ROOT_PATH, 'rpm', 'manifest.json')
...
export const projectCacheStatus = async (args: { stale_days: number }) => {
  const cacheDir = path.join(ROOT_PATH, '.project-cache')
  let entries: Dirent[]
  try {
...
export const debugInfo = async (args: { flag_size_mb: number; flag_age_days: number }) => {
  const debugDir = path.join(ROOT_PATH, 'debug')
