...
- `MCP_CLAUDE_HOUSEKEEPING_AUDIT_LOG_PATH` (optional) — JSONL audit log path. Defaults to `<MCP_CLAUDE_HOUSEKEEPING_PATH>/audit/audit.jsonl`. Each `_cleaner_` tool invocation appends one event with `{ts, server, tool, role, ok, duration_ms, error?, args}` (memory/audit `content` fields and oversized payloads are truncated). Write failures are logged to stderr only and never block the tool call. See [src/shared/audit-log.ts](./src/shared/audit-log.ts).
- `MCP_CLAUDE_HOUSEKEEPING_AUDIT_LOG_ALL` (optional, `1` to enable) — also log `_auditor_` (read-only) tool invocations.
