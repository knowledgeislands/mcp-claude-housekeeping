...
describe('appendAuditEvent / withAuditLog', () => {
  ...
  it('appends an event line for a destructive-level tool', async () => {
    ...
    const wrapped = withAuditLog('test_destructive_tool', 'destructive', async () => ({ content: [{ type: 'text', text: '{}' }] }))
  ...
  it('records ok:false and error text when tool result is isError', async () => {
    const { withAuditLog } = await import('./audit-log.js')
    const wrapped = withAuditLog('test_destructive_tool', 'destructive', async () => ({ isError: true, content: [{ type: 'text', text: 'boom' }] }))
    await wrapped({})
    await new Promise((r) => setTimeout(r, 20))
    const event = JSON.parse((await fs.readFile(logPath, 'utf-8')).trim())
  ...
  it('records ok:false when the handler throws', async () => {
    const { withAuditLog } = await import('./audit-log.js')
    const wrapped = withAuditLog('test_destructive_tool', 'destructive', async () => {
  ...
  it('logs read-level tools when AUDIT_LOG=all', async () => {
  ...
  it('skips every tool — read and destructive levels — when AUDIT_LOG=off', async () => {
    ...
    expect(withAuditLog('test_destructive_tool', 'destructive', destructiveHandler)).toBe(destructiveHandler)
  ...
  it('records ok:false with no error text when an isError result has no content array', async () => {
    ...
    const { withAuditLog } = await import('./audit-log.js')
    const wrapped = withAuditLog('test_destructive_tool', 'destructive', async () => ({ isError: true }))
    await wrapped({})
    await new Promise((r) => setTimeout(r, 20))
    const event = JSON.parse((await fs.readFile(logPath, 'utf-8')).trim())
    expect(event.ok).toBe(false)
