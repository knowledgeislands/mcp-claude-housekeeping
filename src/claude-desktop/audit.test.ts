...
describe('pluginsInventory', () => {
  ...
  it('aggregates installed_plugins.json and rpm/manifest.json', async () => {
    ...
    await fs.writeFile(path.join(CLAUDE_DESKTOP_ROOT_PATH, 'rpm', 'manifest.json'), JSON.stringify({ plugins: [{ id: 'p1', name: 'plugin-1', updatedAt: '2026-04-01T00:00:00Z' }] }))
...
describe('extra branch coverage for desktop audit', () => {
  ...
  it('artifactHealth: handles artifacts with no versions/no updatedAt (last_updated=null, age_days=null)', async () => {
    ...
    const result = await artifactHealth(CLAUDE_DESKTOP_ROOT_PATH, { flag_versions: 100, flag_stale_days: 999, flag_unstarred_idle_days: 999 })
  ...
  it('artifactHealth: uses createdAt when updatedAt is missing', async () => {
    const oneDay = 24 * 60 * 60 * 1000
    await fs.writeFile(path.join(CLAUDE_DESKTOP_ROOT_PATH, 'artifacts.json'), JSON.stringify([{ id: 'created-only', name: 'CO', isStarred: false, createdAt: Date.now() - 100 * oneDay }]))
