...
If you find a security issue in `@knowledgeislands/mcp-housekeeping-claude`, **please do not file a public GitHub issue.** Instead, email the maintainer directly:

- **<kris@kris.me.uk>** — subject: `mcp-housekeeping-claude security`

Include:

- A description of the issue and the impact (e.g. "path traversal", "arbitrary file write outside a workspace").
- Steps to reproduce, ideally with a minimal proof-of-concept.
- The version of the package (`bun pm ls @knowledgeislands/mcp-housekeeping-claude`, or `npm ls ...` if installed via npm) and Node version.
...
- Config loading in `src/config/index.ts` (`loadConfig` — MCP_HOUSEKEEPING_CLAUDE_PATH required) and workspace discovery in `src/utils/utils.ts` (`discoverWorkspaces`).
...
- Misconfiguration of `MCP_HOUSEKEEPING_CLAUDE_PATH` to a directory the user did not intend to expose.
